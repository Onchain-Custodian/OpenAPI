...
The **API key** string is generated by the Custodian platform.

> Each API key is uniquely associated to a wallet. Each wallet can have up to 5 API keys at max. API keys do not expire.
...
|  PUT   | [v1/api/account/collect](#717-collect-funds-from-child-addresses-and-transfer-to-master-address)               | Collect funds from child addresses and transfer to master address when balance is greater than the specified threshold |
|  PUT   | [v1/api/account/collect/auto](#718-update-auto-collect-configuration)                                          | Update auto-collect configuration                                                                                      |
|  PUT   | [v1/api/account/hd/collect](#719-collect-specified-amount-from-a-child-address-and-transfer-to-master-address) | Collect specified amount of an asset from a child address and transfer to master address                               |
...
|    USDC    | USDC              |
|    CGT     | Cache Gold Token  |
|    WTC     | Waltonchain Token |
|    REN     | REN Token         |
|    FTM     | Fantom Token      |
|   MATIC    | Matic Token       |
| XSGD-ERC20 | XSGD on Ethereum  |
|    BCH     | Bitcoin Cash      |
|    ZIL     | Zilliqa Token     |
| XSGD-ZRC2  | XSGD on Zilliqa   |
...
> If no coin type is specified, the method will return the balance for all the coin types in the wallet.
...
| include_master_address | boolean | Whether include the master address (for each coin type) |    no    |
...
> In case no sub-addresses are specified, funds will be collected from all addresses with balance greater than the specified threshold.
...
#### 7.1.9. Collect Specified Amount from a Child Address and Transfer to Master Address
...
| :---------: | :----: | :------------------------------------------------------- | :------: |
|  coin_type  | string | coin type                                                |   yes    |
| sub-address | string | address to collect the amount from                       |   yes    |
|  tx_amount  | number | amount to be collected and transferred to master address |   yes    |
